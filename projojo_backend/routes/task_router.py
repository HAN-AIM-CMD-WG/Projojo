...
async def revert_registration_started(
    ...
    """
    try:
        task_repo.revert_registration_started(task_id, student_id)
        return {"message": "Startstatus van registratie teruggezet"}
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Kon startstatus niet terugzetten: {str(e)}")
...
async def revert_registration_completed(
    ...
    """
    try:
        task_repo.revert_registration_completed(task_id, student_id)
        return {"message": "Voltooiingsstatus van registratie teruggezet"}
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Kon voltooiingsstatus niet terugzetten: {str(e)}")
...
async def get_registration_timeline(
    ...
    if role == "student":
        if user_id != student_id:
            raise HTTPException(status_code=403, detail="Je kunt alleen je eigen registratietijdlijn bekijken")
    elif role == "supervisor":
        task_business_id = task_repo.get_business_id_by_task(task_id)
        if not task_business_id or task_business_id != payload.get("businessId"):
