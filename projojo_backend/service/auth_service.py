...
class AuthService:
    ...
    async def _extract_user_from_token(self, provider: str, client, token) -> User:
        ...
        if provider == 'google':
            ...
            return await self._extract_surf_user(token)
        else:
    ...
    async def _extract_surf_user(self, token) -> User:
        """Extract user info from SURF OAuth token"""
        # TODO: This is generated by copilot. Check if it works after creating SURF app.
        user_info = token.get('userinfo')

        if not user_info:
            raise ValueError("Failed to get user info from SURF")
