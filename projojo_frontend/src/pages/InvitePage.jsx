...
export default function InvitePage() {
    ...
    useEffect(() => {
        if (!token || token.trim().length < 16 || token.trim().length > 64) {
            notification.error('Ongeldige of verlopen uitnodiging');
    ...
    return (
        <div className="w-full min-h-dvh flex items-center justify-center">
            <div className="max-w-md p-6 bg-white rounded-2xl shadow-lg border border-gray-200 text-center">
                ...
                <div className="mt-6 pt-4 border-t border-gray-100">
                    <Link to="/" className="text-gray-500 hover:text-gray-700 text-sm hover:underline flex items-center justify-center gap-2">
                        ...
                        Annuleren en terug naar de loginpagina
                    </Link>
                </div>
            </div>
        </div>
