# TS-task-020 — Fix OverviewPage Public-Only Theme Limitation

**Phase**: 2 — UI Workflows  
**Priority**: 🟠 Medium-High  
**Type**: Functional Task (Bug Fix)  
...
**Then** themes come from the enriched `getBusinessesComplete()` response ([TS-task-009](TS-task-009-backend-themes-in-business-query.md))  
...
- **Fix**: 
  ...
  2. Read themes directly from the enriched `getBusinessesComplete()` response (each project now has a `themes` array per [TS-task-009](TS-task-009-backend-themes-in-business-query.md))
